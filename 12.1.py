...
def search(dirs, filename):
    result = []
    def re_search(subdirs, current_path):
        for node in subdirs:
            if type(node) == str: #Якщо стр, то це або файл, або папка, а папка або порожня або ні
                if node == filename:
                    result.append(f"/{current_path}/{node}")#Зберегаємо рещультат в акум змінній
            else:
                if current_path:# Якщо не порожній
                    new_path = f"{current_path}//{node[0]}" 
                else: #Якщо порожній
                    new_path = node[0]
                re_search(node[1], new_path)#Робимо шаг
    re_search(dirs, "")
    return result #Виводимо
